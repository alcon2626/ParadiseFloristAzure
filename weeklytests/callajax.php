...
if($CustomerID != ''){
  $sql = "SELECT orders.ORDER_ID, orders.CONFIR_NUMB, orders.ITEM_ID, accounts.FNAME, accounts.LNAME, accounts.ADDRESS, accounts.CITY, accounts.ZIP, accounts.COUNTRY
          FROM Orders
          INNER JOIN accounts ON orders.USER_ID = accounts.CUST_ID
          WHERE accounts.CUST_ID = '$CustomerID'";
}else{
